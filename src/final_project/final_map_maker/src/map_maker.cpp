...
public:
  MapMaker() {
    ...
    m_rosNodeHandler.param("map_maker/interval", m_interval_param,
                           (double)0.2);
    m_rosNodeHandler.param("map_maker/dottedCount", m_dottedCount_param,
                           (int)15);
...
public:
  void makeMap() {
    ...
    if (distance_square >= m_interval_param * m_interval_param * 0.9) {

      m_dPrevVehicleX = m_dVehicleX;
      m_dPrevVehicleY = m_dVehicleY;

      if (ignoreCount >= m_dottedCount_param) {
        ignoreCount = 0;
        if (isIgnore == true) {
          isIgnore = false;
        } else {
      ...
      if (isIgnore == false) {
        ...
        for (auto i = 0; i < m_laneCount_param; i++) {
          ...
          try {
            ...
            point.x = lanePoint_world.point.x;
            point.y = lanePoint_world.point.y;
            m_lanes.lane[i].point.push_back(point);
      ...
      for (auto i = 0; i < m_laneCount_param - 1; i++) {
        ...
        try {
          ...
          point.x = lanePoint_world.point.x;
          point.y = lanePoint_world.point.y;
          m_mids.lane[i].point.push_back(point);
        } catch (tf::TransformException &ex) {
          // ROS_ERROR(ex.what());
        }
      }
