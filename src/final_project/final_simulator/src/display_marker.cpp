...
protected:
  ...
  ros::Publisher m_rosPubVehicleBoundaryMarker;
  ...
  std::string m_vehicle_namespace = ros::this_node::getNamespace();
  ...
  Display() {
    m_rosPubVehiclesMarker =
        m_rosNodeHandler.advertise<visualization_msgs::Marker>("vehicle_marker",
                                                               10);
    m_rosPubVehicleBoundaryMarker =
        m_rosNodeHandler.advertise<visualization_msgs::Marker>("vehicle_boundary_marker",
                                                               10);
...
public:
  ...
  //         m_csvLanes.lane.at(i_lane).arclength = arclength;
  //       }
  //       prevPoint = currPoint;
  //     }
  //   }
...
public:
  ...
  void text_vehicle(){
    ...
    modeText.text = "Velocity(KPH) : \n" + std::to_string(3.6*m_dVehicleVel);
  ...
  void mark_vehicle() {
    if (m_isVehicleExist == true) {
      ...
      q_temp.setRPY(0.0 / 180.0 * PI, 0, 90.0 / 180.0 * PI );
      tf::Quaternion q(q_temp.getX(), q_temp.getY(), q_temp.getZ(),
                       q_temp.getW());

      visualization_msgs::Marker vehicle_marker;
      vehicle_marker.header.frame_id = m_vehicle_namespace + "/body";
      ...
      vehicle_marker.pose.position.x = 2.15;//m_dVehicleX;
      vehicle_marker.pose.position.y = 0.06;//m_dVehicleY;
  ...
  void mark_vehicle_boundary() {
    if (m_isVehicleExist == true) {
      ...
      tf::Quaternion q(q_temp.getX(), q_temp.getY(), q_temp.getZ(),
                       q_temp.getW());
      visualization_msgs::Marker vehicle_boundary_marker;
      vehicle_boundary_marker.header.frame_id = m_vehicle_namespace + "/body";
      vehicle_boundary_marker.header.stamp = ros::Time::now();
      vehicle_boundary_marker.ns = m_sVehicle_id;
      vehicle_boundary_marker.id = 0;
      // Set the marker type
      vehicle_boundary_marker.type = visualization_msgs::Marker::CUBE;
      // vehicle_boundary_marker.mesh_resource =
      //     "package://final_simulator/meshes/evoque_new.dae";
          //"package://final_simulator/meshes/BMW_X5_4.dae";
      // vehicle_boundary_marker.mesh_use_embedded_materials = false;

      vehicle_boundary_marker.pose.position.x = 0.0;//0.0;//m_dVehicleX-2.15;
      vehicle_boundary_marker.pose.position.y = 0.0;//m_dVehicleY;
      vehicle_boundary_marker.pose.position.z = 0.8;//0.8;
      vehicle_boundary_marker.pose.orientation.x = q.getX();
      vehicle_boundary_marker.pose.orientation.y = q.getY();
      vehicle_boundary_marker.pose.orientation.z = q.getZ();
      vehicle_boundary_marker.pose.orientation.w = q.getW();
      // Set the scale of the marker
      vehicle_boundary_marker.scale.x = 2.3;
      vehicle_boundary_marker.scale.y = 4.6;
      vehicle_boundary_marker.scale.z = 1.8;

      if(m_vehicle_namespace == "ego"){
        vehicle_boundary_marker.color.r = 0.0;
        vehicle_boundary_marker.color.g = 1.0;
        vehicle_boundary_marker.color.b = 0.5;
        vehicle_boundary_marker.color.a = 0.2;
      }
      else{
        vehicle_boundary_marker.color.r = 0.3;
        vehicle_boundary_marker.color.g = 0.3;
        vehicle_boundary_marker.color.b = 1.0;
        vehicle_boundary_marker.color.a = 0.4;
      }

      vehicle_boundary_marker.lifetime = ros::Duration(0.1);

      m_rosPubVehicleBoundaryMarker.publish(vehicle_boundary_marker);
    }
  }
...
public:
  ...
  void mark_environmentArea(){
    ...
    for(int i_ice=0; i_ice < m_environmentArea.ice_x.size(); i_ice++){
      ...
      marker.color.a = 0.7;
      marker.scale.x = 2.0*m_environmentArea.ice_radius[i_ice];
      marker.scale.y = 2.0*m_environmentArea.ice_radius[i_ice];
      marker.scale.z = 0.05;
      marker.lifetime = ros::Duration();
  ...
  public:
    ...
    void mark_environmentMode(){
      ...
      if(m_environmentMode.ice_mode == "Asphalt"){
        modeText.text = "Road Condition : Asphalt ";
      }else{
        modeText.text = "Road Condition : Ice ";
      } 
