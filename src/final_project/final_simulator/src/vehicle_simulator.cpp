...
Vehicle::Vehicle() {
  ...
  m_rosSubEnvironmentParam_Input =
      m_rosNodeHandler.subscribe("environment_param", 10, &Vehicle::callbackEnvironmentParam, this);
...
#if 1
  m_dFrontAngle_rps = msg->steering;
  if(isnan(m_dFrontAngle_rps)){
    ROS_ERROR_STREAM("sub Nan FROM MSG");
  }
#endif
}

void Vehicle::callbackEnvironmentParam(const autonomous_msg::EnvironmentParam::ConstPtr &msg) {
  m_dFyf_max = msg->fyf_max;
  m_dFyr_max = msg->fyr_max;
  m_dSlope_rad = msg->slope_angle;
}
...
bool Vehicle::simVehicleModel() {
  ...
  if(ros::Time::now().toSec() -m_init_time < 5.0){
    return false;
  }
  else if(ros::Time::now().toSec() - m_dPrevTime > 5.0){
    m_dPrevTime = ros::Time::now().toSec();
  }
...
void Vehicle::simVehicleLateralModel() {
  ...
  // Get Cornering stiffness from param node
  ...
  if (fabs(m_dVehicleVel_ms) >= 5.0) // 5.0 || m_dVehicleVel_ms <= -5.0)
  {
    ...
    if (Fyr > m_dFyr_max) {
      Fyr = m_dFyr_max;
    } else if (Fyr < -1.0 * m_dFyr_max) {
      Fyr = -1.0 * m_dFyr_max;
    }
    // ROS_WARN_STREAM_THROTTLE(0.3,Fyf<<" "<<Fyr);
    ...
    m_dSlipAngle = atan2((m_dLr * tan(m_dFrontAngle_rps)), (m_dLf + m_dLr));
  ...
  double m_dYaw_degree = fmod(m_dYaw*180/PI,360);
  if (m_dYaw_degree<0){ m_dYaw_degree += 360; }
  m_dYaw = m_dYaw_degree*PI/180.0;
